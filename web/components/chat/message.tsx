...
const PurePreviewMessage = ({
  ...
  const parts = message.parts?.map((part, index) => {
    ...
    if (type === "text") {
      // Ollama-compatible providers can emit a tool call plus an empty text
      // part (see PLAN.md §5.3) rather than omitting the text part
      // entirely. Skip rendering it so a tool call doesn't leave a blank
      // bubble floating next to the tool card.
      if (!part.text?.trim()) {
        return null;
      }
      return (
